...
from browser_use_ultra.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError

# CDP logging is now handled by setup_logging() in logging_config.py
# It automatically sets CDP logs to the same level as browser_use_ultra logs
from browser_use_ultra.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
from browser_use_ultra.browser.events import (
...
from browser_use_ultra.browser.profile import BrowserProfile, ProxySettings
from browser_use_ultra.browser.views import BrowserStateSummary, TabInfo
from browser_use_ultra.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from browser_use_ultra.observability import observe_debug
from browser_use_ultra.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from browser_use_ultra.actor.page import Page
	from browser_use_ultra.browser.demo_mode import DemoMode
	from browser_use_ultra.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from browser_use_ultra.skill_cli.utils import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
		...
		if profile_directory is None:
			if profiles:
				...
				logging.getLogger('browser_use_ultra').info(
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from browser_use_ultra.skill_cli.utils import list_chrome_profiles
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'browser_use_ultra.{self}')
		return logging.getLogger(f'browser_use_ultra.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from browser_use_ultra.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from browser_use_ultra.browser.events import SaveStorageStateEvent
				...
				if self.browser_profile.use_cloud or self.browser_profile.cloud_browser_params is not None:
					...
					try:
						...
						raise CloudBrowserAuthError(
							'Authentication failed for cloud browser service. Set BROWSER_USE_API_KEY environment variable. You can also create an API key at https://cloud.browser-use-ultra.com/new-api-key'
						)
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from browser_use_ultra.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from browser_use_ultra.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from browser_use_ultra.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from browser_use_ultra.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from browser_use_ultra.browser.crash_watchdog import CrashWatchdog
		from browser_use_ultra.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from browser_use_ultra.browser.watchdogs.dom_watchdog import DOMWatchdog
		from browser_use_ultra.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from browser_use_ultra.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from browser_use_ultra.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from browser_use_ultra.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from browser_use_ultra.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from browser_use_ultra.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from browser_use_ultra.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from browser_use_ultra.browser.watchdogs.security_watchdog import SecurityWatchdog
		from browser_use_ultra.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from browser_use_ultra.browser.session_manager import SessionManager
			...
			from browser_use_ultra.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from browser_use_ultra.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from browser_use_ultra.dom.views import NodeType
	...
	async def remove_highlights(self) -> None:
		...
		try:
			async with asyncio.timeout(3.0):
				...
				script = """
				(function() {
					// Remove all browser-use-ultra highlight elements
					const highlights = document.querySelectorAll('[data-browser-use-ultra-highlight]');
					console.log('Removing', highlights.length, 'browser-use-ultra highlight elements');
					highlights.forEach(el => el.remove());

					// Also remove by ID in case selector missed anything
					const highlightContainer = document.getElementById('browser-use-ultra-debug-highlights');
					...
					const orphanedTooltips = document.querySelectorAll('[data-browser-use-ultra-highlight="tooltip"]');
	...
	async def highlight_interaction_element(self, node: 'EnhancedDOMTreeNode') -> None:
		...
		try:
			...
			(function() {{
				...
				container.setAttribute('data-browser-use-ultra-interaction-highlight', 'true');
	...
	async def highlight_coordinate_click(self, x: int, y: int) -> None:
		...
		try:
			...
			(function() {{
				...
				// Create container
				const container = document.createElement('div');
				container.setAttribute('data-browser-use-ultra-coordinate-highlight', 'true');
				container.style.cssText = `
					position: absolute;
					left: ${{x + scrollX}}px;
					top: ${{y + scrollY}}px;
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				const existingContainer = document.getElementById('browser-use-ultra-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-browser-use-ultra-highlight]');
				...
				const container = document.createElement('div');
				container.id = 'browser-use-ultra-debug-highlights';
				container.setAttribute('data-browser-use-ultra-highlight', 'container');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-browser-use-ultra-highlight', 'element');
