...
if len(sys.argv) > 1 and sys.argv[1] == 'install':
	...
	if result.returncode == 0:
		...
		print('🚀 Ready to use! Run: uvx browser-use-ultra')
	else:
		print('\n❌ Installation failed')
		sys.exit(1)
	sys.exit(0)
...
if 'init' in sys.argv:
	from browser_use_ultra.init_cmd import INIT_TEMPLATES
	from browser_use_ultra.init_cmd import main as init_main
...
if '--template' in sys.argv:
	...
	output_path = Path(output) if output else Path.cwd() / f'browser_use_ultra_{template}.py'
	...
	try:
		...
		click.echo('\nNext steps:')
		click.echo('  1. Install browser-use-ultra:')
		click.echo('     uv pip install browser-use-ultra')
		click.echo('  2. Set up your API key in .env file or environment:')
		click.echo('     BROWSER_USE_API_KEY=your-key')
		click.echo('     (Get your key at https://cloud.browser-use-ultra.com/new-api-key)')
...
from browser_use_ultra import Agent, Controller
from browser_use_ultra.agent.views import AgentSettings
from browser_use_ultra.browser import BrowserProfile, BrowserSession
from browser_use_ultra.logging_config import addLoggingLevel
from browser_use_ultra.telemetry import CLITelemetryEvent, ProductTelemetry
from browser_use_ultra.utils import get_browser_use_ultra_version
...
try:
	...
	print(
		'⚠️ CLI addon is not installed. Please install it with: `pip install "browser-use-ultra[cli]"` and try again.', file=sys.stderr
	)
	sys.exit(1)
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('browser_use_ultra.'):
		...
		# Configure browser_use_ultra logger and all its sub-loggers
		browser_use_ultra_logger = logging.getLogger('browser_use_ultra')
		browser_use_ultra_logger.propagate = False  # Don't propagate to root logger
		browser_use_ultra_logger.handlers = [log_handler]  # Replace any existing handlers
		browser_use_ultra_logger.setLevel(root.level)

		# Also ensure agent loggers go to the main output
		# Use a wildcard pattern to catch all agent-related loggers
		for logger_name in ['browser_use_ultra.Agent', 'browser_use_ultra.controller', 'browser_use_ultra.agent', 'browser_use_ultra.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'browser_use_ultra.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('browser_use_ultra.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('browser_use_ultra.app')
	...
	async def action_quit(self) -> None:
		...
		self.exit()
		print('\nTry running tasks on our cloud: https://browser-use-ultra.com')
		...
		with Container(id='main-container'):
			...
			with Container(id='links-panel'):
				with HorizontalGroup(classes='link-row'):
					...
					yield Link('https://browser-use-ultra.com', url='https://browser-use-ultra.com', classes='link-white link-url')
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/browser-use-ultra/awesome-prompts',
						url='https://github.com/browser-use-ultra/awesome-prompts',
				...
				with HorizontalGroup(classes='link-row'):
					...
					yield Link(
						'https://github.com/browser-use-ultra/browser-use-ultra/issues',
						url='https://github.com/browser-use-ultra/browser-use-ultra/issues',
...
async def run_prompt_mode(prompt: str, ctx: click.Context, debug: bool = False):
	"""Run browser-use-ultra in non-interactive mode with a single prompt."""
...
async def textual_interface(config: dict[str, Any]):
	...
	# Prevent browser_use_ultra from setting up logging at import time
	os.environ['BROWSER_USE_SETUP_LOGGING'] = 'false'

	logger = logging.getLogger('browser_use_ultra.startup')
	...
	try:
		...
		try:
			from browser_use_ultra.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from browser_use_ultra.sync.auth import DeviceAuthClient
	...
	try:
		...
		from browser_use_ultra.sync.service import CloudSync
		...
		task_event = CreateAgentTaskEvent(
			...
			llm_model='auth-flow',
			task='🔐 Complete authentication and join the browser-use-ultra community',
			user_id=auth_client.temp_user_id,
			device_id=auth_client.device_id,
			done_output=None,
		...
		try:
			...
			from browser_use_ultra.utils import create_task_with_error_handling
		...
		if success:
			...
			step_event = CreateAgentStepEvent(
				...
				actions=[
					{
						...
						'done': {
							'success': True,
							'text': '⭐ Starred browser-use-ultra/browser-use-ultra repository! Welcome to the community!',
						},
					}
				],
				next_goal='⭐ Star browser-use-ultra GitHub repository to join the community',
				...
				screenshot_url=None,
				url='https://github.com/browser-use-ultra/browser-use-ultra',
			)
			...
			print('   Future browser-use-ultra runs will now sync to the cloud.')
		else:
		...
		if task_id and sync_service:
			try:
				from browser_use_ultra.agent.cloud_events import UpdateAgentTaskEvent
...
@click.option('--output', '-o', type=click.Path(), help='Output file path for template (default: browser_use_ultra_<template>.py)')
...
def main(ctx: click.Context, debug: bool = False, **kwargs):
	...
	Examples:
	  uvx browser-use-ultra --template default
	  uvx browser-use-ultra --template advanced --output my_script.py
	"""
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	"""Run the main browser-use-ultra interface"""

	if kwargs['version']:
		from importlib.metadata import version

		print(version('browser-use-ultra'))
		sys.exit(0)
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from browser_use_ultra.mcp.server import main as mcp_main
...
def _run_template_generation(template: str, output: str | None, force: bool):
	...
	if output:
		...
		output_path = Path.cwd() / f'browser_use_ultra_{template}.py'
...
@click.option(
	...
	'-o',
	type=click.Path(),
	help='Output file path (default: browser_use_ultra_<template>.py)',
)
@click.option(
	'--force',
	'-f',
...
def init(
	...
	"""
	Generate a browser-use-ultra template file to get started quickly.

	Examples:

	\b
	# Interactive mode - prompts for template selection
	uvx browser-use-ultra init

	\b
	# Generate default template
	uvx browser-use-ultra init --template default

	\b
	# Generate advanced template with custom filename
	uvx browser-use-ultra init --template advanced --output my_script.py

	\b
	# List available templates
	uvx browser-use-ultra init --list
	"""
