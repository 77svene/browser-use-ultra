...
if TYPE_CHECKING:
	from browser_use_ultra.skills.views import Skill

from dotenv import load_dotenv

from browser_use_ultra.agent.cloud_events import (
...
from browser_use_ultra.agent.message_manager.utils import save_conversation
from browser_use_ultra.llm.base import BaseChatModel
from browser_use_ultra.llm.exceptions import ModelProviderError, ModelRateLimitError
from browser_use_ultra.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from browser_use_ultra.tokens.service import TokenCost
...
from browser_use_ultra import Browser, BrowserProfile, BrowserSession
from browser_use_ultra.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from browser_use_ultra.agent.gif import create_history_gif
from browser_use_ultra.agent.message_manager.service import (
	MessageManager,
)
from browser_use_ultra.agent.prompts import SystemPrompt
from browser_use_ultra.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from browser_use_ultra.browser.events import _get_timeout
from browser_use_ultra.browser.session import DEFAULT_BROWSER_PROFILE
from browser_use_ultra.browser.views import BrowserStateSummary
from browser_use_ultra.config import CONFIG
from browser_use_ultra.dom.views import DOMInteractedElement, MatchLevel
from browser_use_ultra.filesystem.file_system import FileSystem
from browser_use_ultra.observability import observe, observe_debug
from browser_use_ultra.telemetry.service import ProductTelemetry
from browser_use_ultra.telemetry.views import AgentTelemetryEvent
from browser_use_ultra.tools.registry.views import ActionModel
from browser_use_ultra.tools.service import Tools
from browser_use_ultra.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_browser_use_ultra_version,
	get_browser_use_ultra_version,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from browser_use_ultra.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from browser_use_ultra import ChatBrowserUse

				llm = ChatBrowserUse()

		# set flashmode = True if llm is ChatBrowserUse
		if llm.provider == 'browser-use-ultra':
			flash_mode = True
		...
		supports_coordinate_clicking = any(
			pattern in model_name for pattern in ['claude-sonnet-4', 'claude-opus-4', 'gemini-3-pro', 'browser-use-ultra/']
		)
		...
		if skill_service is not None:
			...
			from browser_use_ultra.skills import SkillService
		...
		self.agent_directory = base_tmp / f'browser_use_ultra_agent_{self.id}_{timestamp}'
		...
		self._set_browser_use_ultra_version_and_source(source)
		...
		# Check if model is a browser-use-ultra fine-tuned model (uses simplified prompts)
		is_browser_use_ultra_model = 'browser-use-ultra/' in self.llm.model.lower()
		...
		self._message_manager = MessageManager(
			...
			system_message=SystemPrompt(
				...
				is_browser_use_ultra_model=is_browser_use_ultra_model,
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'browser_use_ultra.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from browser_use_ultra.screenshots.service import ScreenshotService
	...
	def _set_browser_use_ultra_version_and_source(self, source_override: str | None = None) -> None:
		"""Get the version from pyproject.toml and determine the source of the browser-use-ultra package"""
		# Use the helper function for version detection
		version = get_browser_use_ultra_version()
	...
	async def _judge_trace(self) -> JudgementResult | None:
		...
		if self.judge_llm.provider == 'browser-use-ultra':
		...
		if self.history.history[-1].result[-1].is_done:
			...
			if judgement:
				...
				if judgement.reached_captcha:
					...
					judge_log += '   👉 🥷 Use Browser Use Cloud for the most stealth browser infra: https://docs.browser-use-ultra.com/customize/browser/remote\n'
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from browser_use_ultra.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		if CONFIG.BROWSER_USE_VERSION_CHECK:
			latest_version = await check_latest_browser_use_ultra_version()
			if latest_version and latest_version != self.version:
				self.logger.info(
					f'📦 Newer version available: {latest_version} (current: {self.version}). Upgrade with: uv add browser-use-ultra=={latest_version}'
				)
	...
	def _log_first_step_startup(self) -> None:
		...
		if len(self.history.history) == 0:
			self.logger.info(
				f'Starting a browser-use-ultra agent with version {self.version}, with provider={self.llm.provider} and model={self.llm.model}'
			)
	...
	def _log_final_outcome_messages(self) -> None:
		...
		if is_successful is False or is_successful is None:
			...
			self.logger.info('   Open a short issue on GitHub: https://github.com/browser-use-ultra/browser-use-ultra/issues')
	...
	async def run(
		...
		from browser_use_ultra.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from browser_use_ultra.agent.views import RerunSummaryAction
		...
		from browser_use_ultra.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from browser_use_ultra.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from browser_use_ultra.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from browser_use_ultra.llm.messages import SystemMessage, UserMessage
		from browser_use_ultra.utils import sanitize_surrogates
		...
		try:
			from browser_use_ultra.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from browser_use_ultra.agent.variable_detector import detect_variables_in_history
